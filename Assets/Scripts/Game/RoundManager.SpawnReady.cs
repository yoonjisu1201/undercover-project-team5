...
public partial class RoundManager
{
    ...
    [Header("스폰 대기 제한 시간 (초) — 초과 시 미보고 클라이언트를 세션에서 내보낸다")]
    [SerializeField] private float _spawnReadyTimeoutSeconds = 20f;

    private const string SpawnReadyTimeoutReason = "게임 준비 시간이 초과되었습니다";
    private const string HostNotReadyReason = "방장의 게임 준비가 완료되지 않았습니다";
    private static readonly string NotEnoughPlayersReason =
        $"게임 시작 인원이 부족합니다 (최소 {WaitingRoomReadyManager.MinPlayersToStart}명)";
    ...
    // 특정 클라이언트의 준비 보고가 오지 않아도 전원이 무한 로딩에 갇히지 않도록,
    // 제한 시간이 지나면 미보고 클라이언트를 내보내고 남은 인원으로 진행 여부를 정한다.
    private async UniTaskVoid WatchSpawnReadyTimeoutAsync(CancellationToken cancellationToken)
    {
        ...
        await UniTask.Delay(TimeSpan.FromSeconds(_spawnReadyTimeoutSeconds), DelayType.Realtime,
            cancellationToken: cancellationToken);
        ...
        if (!_spawnReadyConfirmedClients.Contains(NetworkManager.LocalClientId))
        {
            ...
            SendEveryoneToLobby(HostNotReadyReason);
            return;
        }
        ...
        if (NetworkManager.ConnectedClientsIds.Count < WaitingRoomReadyManager.MinPlayersToStart)
        {
            SendEveryoneToLobby(NotEnoughPlayersReason);
            return;
        }
