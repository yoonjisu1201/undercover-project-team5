...
public abstract class EntranceDoor : InteractableBase {
	...
	// 들어가는 문과 나오는 문의 소리가 달라야 한다. 파생 클래스가 자기 방향을 정한다.
	protected abstract SoundKey DoorSoundKey { get; }
	...
	public override void Interact(GameObject interactor) {
		...
		// 위치 없이 즉시 낸다.
		SoundManager.Instance?.Play(DoorSoundKey);

		// 남아 있는 사람들은 문 위치에서 듣는다. 누가 지하로 내려갔는지 소리로 알 수 있어야 한다.
		RequestDoorSoundRpc(transform.position, DoorSoundKey);
	...
	[Rpc(SendTo.Server)]
	private void RequestDoorSoundRpc(Vector3 position, SoundKey soundKey, RpcParams rpcParams = default)
	{
		// 보낸 본인은 위에서 이미 자기 소리를 냈다. 다시 보내면 겹친다.
		PlayDoorSoundRpc(position, soundKey, RpcTarget.Not(rpcParams.Receive.SenderClientId, RpcTargetUse.Temp));
	}

	[Rpc(SendTo.SpecifiedInParams)]
	private void PlayDoorSoundRpc(Vector3 position, SoundKey soundKey, RpcParams rpcParams = default)
	{
		SoundManager.Instance?.PlayAt(soundKey, position);
	}
}
