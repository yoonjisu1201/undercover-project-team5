...
public class MontageSyncManager : MontageSyncBase {
	...
	public void RequestSetCloth(ClothPart part, int clothId) {
		if (!IsSpawned) {
	...
	[Rpc(SendTo.Server)]
	private void RequestSetClothRpc(ClothPart part, int clothId, RpcParams rpcParams = default) {
		...
		if (clothId != MontageState.None && ClothCatalog.Find(part, clothId) == null) {
