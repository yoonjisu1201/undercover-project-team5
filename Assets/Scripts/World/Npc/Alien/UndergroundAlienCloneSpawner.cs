...
// 지하(Basement) 구역용 외계인 분신 스포너. GroundAlienCloneSpawner와 같은 방식(라운드 타이머 경과량 기준
// 주기적 보충, 라운드 시작 시 즉시 채우기)으로 동작하지만, 동시에 살아있는 개체를 1마리로만 유지한다.
// 지상과 달리 Basement MapRegion은 필드 구역의 해금 추첨과 무관하게 항상 별도로 관리되므로,
// 스폰 시도 전에 이 스포너가 직접 전용 MapRegionController의 NavMesh 캐시를 갱신해야 한다.
public class UndergroundAlienCloneSpawner : MonoBehaviour
{
    ...
    [SerializeField] private MapRegionController _mapRegionController; // Basement 전용 MapRegionController
    [SerializeField, Min(1)] private int _spawnCountPerCycle = 1;
    [SerializeField, Min(0.1f)] private float _spawnInterval = 60f; // 라운드 타이머가 이만큼(초) 줄어들 때마다 스폰
    [SerializeField, Min(0f)] private float _minimumPlayerDistance = 10f; // 이 거리 안에는 스폰하지 않는다 (지하는 통로가 좁아 지상보다 작게 잡는다)
    ...
    private GameObject GetRoundClonePrefab()
    {
        ...
        {
            Debug.LogError("[UndergroundAlienCloneSpawner] CriminalNpcManager 참조가 없어 이번 라운드의 외계인 종류를 알 수 없습니다.", this);
            return null;
        }
        ...
        {
            Debug.LogError(
                $"[UndergroundAlienCloneSpawner] 이번 라운드 외계인 종류({typeIndex})에 해당하는 분신 프리팹이 없습니다. " +
        ...
        if (prefab == null)
        {
            Debug.LogError($"[UndergroundAlienCloneSpawner] {typeIndex}번 외계인 분신 프리팹 슬롯이 비어 있습니다.", this);
        }
    ...
    private bool SpawnClone(GameObject clonePrefab)
    {
        ...
        try
        {
            ...
            {
                Debug.LogError("[UndergroundAlienCloneSpawner] 외계인 프리팹에 NetworkObject 또는 AlienCloneHealth가 없습니다.", this);
                Destroy(instance);
                return false;
            }
        ...
        {
            Debug.LogError($"[UndergroundAlienCloneSpawner] 외계인 스폰 중 예외가 발생해 자동 스폰을 중단합니다.\n{exception}", this);
            _spawnFailedThisCycle = true;
            if (instance != null)
            {
                Destroy(instance);
    ...
    // 지하 맵 안의 임의 NavMesh 지점을 찾는다. 플레이어 근처에 생성하지 않아,
    ...
    private void HandleRoundStateChanged(RoundState state)
    {
        ...
        if (state == RoundState.InRound)
        {
            ...
            _mapRegionController?.RefreshSpawnAreas();
    ...
    // 라운드 시작 시 최대 마릿수(1마리)를 즉시 채운다. 직접 스폰하지 않고 스폰 주기를 다 찬 상태로 만들어
